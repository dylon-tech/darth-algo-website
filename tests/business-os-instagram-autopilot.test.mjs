...
try {
 ...
 execFileSync('node_modules/.bin/tsc',['--target','ES2020','--module','commonjs','--moduleResolution','node','--jsx','react-jsx','--esModuleInterop','--skipLibCheck','--rootDir','app','--outDir',dir,'app/lib/business-os/media-autopilot.ts','app/lib/business-os/telegram-media.ts'],{stdio:'pipe'});
 ...
 assert.equal((await database.query("select count(*)::int as n from os_activity where event='daily_social_status'")).rows[0].n,1);
 ...
 assert.equal((await publishCommunityPreview()).reason,'delivery_uncertain_check_telegram');await publishCommunityPreview();assert.equal(telegramWrites,2);
