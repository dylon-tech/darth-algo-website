...
- Reusable 18-second portrait product promo, 1080x1920 / 24 fps, silent text-first edit. This is recorded chart imagery, not a live signal feed. Native composition source: scripts/darth-tools-promo.jsx.
  ...
  Status: rendered and visually checked; not scheduled or published. Add it through a video-aware publisher so it shares the daily cadence and receipt accounting.
...
- vidIQ: ChatGPT's connector is authorized, but the production research service needs a separate scoped MCP API key. The latest Connections page validates and encrypts the key. A ChatGPT connection does not transfer credentials to the server.
