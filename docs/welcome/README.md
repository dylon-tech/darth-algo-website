...
**NOT LIVE FOR MESSAGING:** @darth.algo is connected in Manychat. A real native keyword flow is saved as a DRAFT; no real message has been sent. Follow-to-DM is explicitly unavailable for this account. `productionAdapters` remains empty: Manychat is the sole designated Instagram sending owner, with no backend duplicate sender. The provider draft must pass an owner-controlled recipient test and final offer validation before activation. The owner now handles Stripe configuration. No paid upgrade, public post, profile change or historical follower import occurred.

The dashboard records a dated manual provider observation, not continuous connection health or live telemetry. The observation imports once into SQL and never overwrites a later sender configuration. Provider send counts stay unavailable. Pause/Edit/Test/Resume cannot falsely acknowledge provider actions: use the authenticated native flow link. The existing 9 AM/3 PM publishing schedule is unchanged. X and TikTok welcome sending remain off while the owner prioritizes Instagram.

## Instagram provider receipt — 2026-09-23 10:14 UTC

- Account: @darth.algo, observed in authenticated Instagram settings, workspace **Darth Algo | Futures Indicator** (`fb5646156`). This workspace ID is not a Meta business account ID and is not stored as one.
- Actual plan/capacity: **Free; contact limit 0/25**. No trial or plan purchase. $0 new spend; recheck capacity before activation.
- Follow-to-DM: **Unavailable yet**; provider says **Meta limited the use of this functionality**. No follower trigger enabled; buying a plan is not a remedy established by this evidence.
- Native saved flow: **Welcome Agent — WELCOME opt-in**, `content20260923101152_967121`, https://app.manychat.com/fb5646156/cms/files/content20260923101152_967121/edit . Reload verified **DRAFT / Saved**, exact case-insensitive **Message is WELCOME → Send Message**, **Within messaging window**. No reminders, paid AI, email collection or further step. Set Live was not clicked.
- Offer contains WELCOME, 25%, TradingView indicators, UTM parameters before #pricing, STOP, and the approved intended first-paid/first-time terms. Those terms are draft requirements, not evidence that the owner's latest Stripe edits were verified.
- Native STOP/Unsubscribe system keywords and **Set Instagram Opt-Out** action observed; recipient suppression, START re-subscription, repeat requests and queue behavior have NOT been account-tested. No recipient/message ID exists.
- Default reply remains stopped. No native status/event integration is exposed to the dashboard; automated permission-expiry monitoring and support handoff for this native flow remain unverified. The inbox needs owner monitoring; no AI support response is claimed.
- Evidence: `instagram-follow-to-dm-status.jpg` and `instagram-welcome-draft.jpg`, saved with the task. Desktop screenshots only; no physical-device or mobile viewport result claimed.
- Resume: owner-controlled Instagram test recipient + final offer validation; test WELCOME, STOP, repeat request and blocked/expired-window behavior before Set Live. Preserve one provider sending owner. If unsupported controls cannot be validated, retain draft.
- Rollback: keep native flow inactive (or stop it in Manychat if later activated); dashboard controls do not stop a provider. Revert this UI/observation release if needed; retain additive SQL evidence. The one-time observation ID must not be reused to force new settings.

`app/lib/welcome/inventory.ts` contains the platform capability record. Historical Stripe audits below are dated snapshots; they do not represent a new request to change Stripe or the current offer configuration.
...
- No registered sender means no production external messaging call is possible. Native provider owners bypass backend sending. Native delivery/pause/STOP telemetry is unavailable; controls are handled directly in Manychat through the saved flow link. Backend controls explicitly report that no provider action was performed.
