...
export default function LabPanel({active}:{active:boolean}){
 ...
 return <section aria-label='Indicator drafts'>
  ...
  <div className={styles.labGrid}>{data?.candidates.map(c=><Link className={styles.labCard} href={`/owner/indicators/${c.id}`} key={c.id}>
   {c.capture_id?<><Image src={`/api/owner/indicators/${c.id}/captures/${c.capture_id}`} alt={`${c.name||'Indicator'} · exact source version chart`} width={720} height={450} unoptimized/><small>{c.capture_origin==='owner_submission'?'Owner-supplied chart':'TradingView capture'} · release testing is separate</small></>:<div className={styles.labEmpty}><FlaskConical size={28}/><span>{c.capture_error?'Chart capture needs attention':'Chart preview pending'}</span><small>{c.capture_error==='TRADINGVIEW_LOGIN_REQUIRED'?'Connect TradingView to capture this version.':c.capture_error?'Open draft for the connection or testing requirement.':'Open draft for source and testing status'}</small></div>}
   <div><small>{c.status==='qa_blocked'?'Chart test needed':c.status==='released'?'Release recorded':c.status==='approved'?'Approved · awaiting release':c.status.replaceAll('_',' ')}</small><h3>{c.name||'Indicator draft'}</h3><p>{c.purpose||'Research and validation are available inside.'}</p><small>Source {c.source_hash?.slice(0,8)||'unverified'} · Release requires your approval <ChevronRight size={15}/></small></div>
  </Link>)}</div>
