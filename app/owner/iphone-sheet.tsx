...
export default function IphoneSheet({children,onClose}:{children:ReactNode;onClose:()=>void}) {
 const root=useRef<HTMLDivElement>(null),start=useRef<number|null>(null);
 const [drag,setDrag]=useState(0),[viewport,setViewport]=useState<{height:number;top:number}|null>(null);
 useEffect(()=>{
  const body=document.body,scrollY=window.scrollY,prior=document.activeElement as HTMLElement|null;
  ...
  const key=(event:KeyboardEvent)=>{
   ...
   if(event.key==='Tab'){const items=focusables(),first=items[0],last=items.at(-1);if(!first){event.preventDefault();return;}if(event.shiftKey&&document.activeElement===first){event.preventDefault();last?.focus();}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus();}}
  };
  document.addEventListener('keydown',key);
  return()=>{cancelAnimationFrame(frame);document.removeEventListener('keydown',key);window.visualViewport?.removeEventListener('resize',visible);window.visualViewport?.removeEventListener('scroll',visible);window.removeEventListener('resize',visible);Object.assign(body.style,saved);window.scrollTo({top:scrollY,behavior:'instant'});requestAnimationFrame(()=>prior?.isConnected&&prior.focus({preventScroll:true}));};
 },[onClose]);
