...
import { revenueGoals } from "../lib/business-os/business-focus";
import { workSummary } from "../lib/business-os/work-summary";
import styles from "./owner.module.css";
import InstallApp from "./install-app";
import BufferPanel from "./buffer-panel";
import { mediaAutopilot } from "../lib/business-os/media-policy";
import Image from "next/image";
import Link from "next/link";
import { isDailySocialPayload } from "../lib/business-os/daily-social-policy";
import { isInstagramPublication } from "../lib/business-os/instagram-policy";
import { isBufferPublication } from "../lib/business-os/buffer-publication-policy";
import type { BufferConnection } from "../lib/business-os/buffer";
const isPublication = (value: unknown) => isBufferPublication(value) || isInstagramPublication(value) || isDailySocialPayload(value);
const hasPublicationImages = (value: unknown) => isInstagramPublication(value) || isDailySocialPayload(value);
const publicationNetwork = (value: unknown) => isDailySocialPayload(value) ? ({x:"X",instagram:"Instagram",threads:"Threads"}[value.network]) : isInstagramPublication(value) ? "Instagram" : "X";
type Agent = {
    id: Department;
    mandate: string;
    state: string;
...
type Approval = {
    delivery?: { state: string; postId?: string; published?: boolean; checkedAt?: string } | null;
    id: string;
    payload_hash: string;
...
type Coordination = {
    ...
    heartbeat: null | { lastSeenAt: string; status: string; workerId: string };
    pendingHandoffs: number;
    completedHandoffs: number;
    recentHandoffs: Array<{ id: string; from_department: string; to_department: string; status: string; created_at: string }>;
    budget: { configured: boolean; dailyLimitUsd?: number; monthlyLimitUsd?: number };
};
type Snapshot = {
...
export default function CommandCenter() {
    ...
    async function action(body: Record<string, unknown>) {
        ...
        try {
            ...
            await refresh();
            if (body.operation === "initialize" || body.operation === "buffer_draft_test")
    ...
    const paymentData = readSource("stripe_payments_60d");
    const paymentTotals = (paymentData?.data as { currencyMinorUnits?: Record<string, {current:number;previous:number}> } | undefined)?.currencyMinorUnits;
    const usdPayments = paymentTotals?.usd;
    const conversions = readSource("paid_conversion")?.data as {taggedSessions:number;untaggedSessions:number;groups:Array<{paidCheckouts:number;trialStarts:number}>} | undefined;
    const taggedPaid = conversions?.groups.reduce((sum,g)=>sum+g.paidCheckouts,0);
    const taggedTrials = conversions?.groups.reduce((sum,g)=>sum+g.trialStarts,0);
    ...
    if (!auth)
        return <main id="main-content" className={styles.login}><div className={styles.loginCard}><div className={styles.eyebrow}><LockKeyhole size={16}/> JUST FOR YOU</div><h1>Darth Algo<span>Your crew. One little HQ.</span></h1><p>Open your private AI Command Center chat in Telegram and send /connect. Tap the button it sends, then Connect this device.</p><p className={styles.muted}>Already connected in Safari? Tap the compass icon to open this page there. Each browser needs its own connection.</p>{!enabled && <p>Owner access is not switched on here yet.</p>}{error && <p role="alert" className={styles.error}>{error}</p>}<small>No passcode or passkey to type.</small></div></main>;
    ...
    {section === "Home" && <>
      <section className={styles.hero}><div className={styles.heroCopy}><span className={styles.eyebrow}>DARTH ALGO • OWNER MODE</span><h1>More customers.<br /><span>More revenue.</span></h1><p>Every agent has one mission: solve the problems that help Darth Algo grow.</p><button className={styles.primary} onClick={() => chat("ceo")}><MessageCircle size={21}/>Talk to my CEO<ArrowUpRight size={19}/></button></div><div className={styles.crewArt} aria-hidden="true"><span className={styles.starOne}>✦</span><div className={styles.robot}><PixelCrew /><span>CEO</span></div><div className={styles.satellite}><Rocket size={28}/></div><div className={styles.satelliteTwo}><Heart size={25}/></div><span className={styles.starTwo}>✦</span><span className={styles.orbitLabel}>YOUR CREW • YOUR CALL</span></div></section>
      ...
      <section aria-label="What your agents are doing"><div className={styles.pageTitle}><h2>Your crew at work</h2><p>See the job. Tap to talk. Open the work steps for real updates.</p><small>{fresh ? "Updates every 15 seconds while this page is open" : "Checking for a fresh update…"} · Goals below are intended benefits, not measured results.</small></div><div className={styles.agentGrid}>{(Object.keys(names) as Department[]).map(id => { const work = workSummary(id, snapshot?.jobs || [], fresh, clock); return <article className={styles.agentCard} key={id}><button className={styles.agentOpen} onClick={() => chat(id)}><div className={styles.panelHeading}><span className={`${styles.avatar} ${styles[colors[id]]}`}><PixelCrew role={id}/></span><Status value={work.label} tone={work.tone}/></div><h2>{names[id]}</h2><p><strong>{work.title}</strong></p><p className={styles.stateDetail}>Goal: {revenueGoals[id]}</p><span className={styles.cardLink}>Talk to {names[id]}<MessageCircle size={18}/></span></button><div className={styles.buttonRow}><button onClick={() => { setDepartment(id); setSection("Messages"); setChatView("work"); }}>See work steps</button><button onClick={() => { setDepartment(id); setSection("Messages"); setChatView("assign"); }}>Give work</button></div></article>; })}</div></section>
      <CoordinationPanel coordination={snapshot?.coordination} now={clock} fresh={fresh} paused={snapshot?.paused ?? true} />
    ...
    {section === "Messages" && <><div className={styles.pageTitle}><span className={styles.eyebrow}>YOUR CREW IS ONE TAP AWAY</span><h1>Messages</h1><p>Pick a character. Ask a question. Watch the work.</p></div><div className={styles.inbox}><aside className={styles.contacts} aria-label="Agent conversations">{(Object.keys(names) as Department[]).map(id => { const latest = snapshot?.messages.find(m => m.department === id); return <button key={id} aria-pressed={department === id} className={department === id ? styles.contactSelected : ""} onClick={() => { setDepartment(id); setChatView("chat"); }}><span className={`${styles.avatar} ${styles[colors[id]]}`}><PixelCrew role={id}/></span><span><b>{names[id]}</b><small>{latest?.body || missions[id]}</small><em>{agentState(id).label}</em></span></button>; })}</aside><div className={styles.inboxMain}><section className={styles.conversation}><div className={styles.chatHeader}><div className={styles.chatIdentity}><span className={`${styles.avatar} ${styles[colors[department]]}`}><PixelCrew role={department}/></span><div><h1>{names[department]}</h1><p>{missions[department]}</p></div></div><Status value={agentState(department).label} tone={agentState(department).tone}/></div><div className={styles.chatTabs}><button aria-pressed={chatView === "chat"} onClick={() => setChatView("chat")}><MessageCircle size={17}/>Messages</button><button aria-pressed={chatView === "assign"} onClick={() => setChatView("assign")}><Rocket size={17}/>Give work</button><button aria-pressed={chatView === "work"} onClick={() => setChatView("work")}><Play size={17}/>Watch work</button></div>{chatView === "chat" ? <><div className={styles.messages} role="log" aria-label={`${names[department]} conversation`}>{!snapshot?.messages.some(m => m.department === department) && !departmentJobs.some(j => j.status === "queued") && <div className={styles.empty}><MessageCircle size={35}/><h2>What shall we work on?</h2><p>{department === "ceo" ? "Try: What should we focus on to win our next customer?" : `Give ${names[department]} one thing to focus on.`}</p></div>}{[...(snapshot?.messages || [])].filter(m => m.department === department).reverse().map(m => <article key={m.id} className={m.role === "owner" ? styles.ownerMessage : styles.agentMessage}><small>{m.role === "owner" ? "You" : names[department]} · {time(m.created_at)}</small><p>{m.body}</p></article>)}{departmentJobs.filter(j => j.status === "queued").slice().reverse().map(j => <article className={styles.ownerMessage} key={j.id}><small>You · {time(j.created_at)} · Saved, waiting to run</small><p>{j.message}</p></article>)}</div><div className={styles.suggestions}><small>QUICK MESSAGES · TAP TO EDIT, THEN SEND</small><div>{prompts.map(prompt => <button key={prompt.title} onClick={() => setMessage(prompt.message)}>{prompt.title}<ArrowUpRight size={15}/></button>)}</div></div><form onSubmit={send} className={styles.composer}><label htmlFor="crew-message">Your message</label><textarea id="crew-message" value={message} onChange={e => setMessage(e.target.value)} maxLength={4000} placeholder={`Hey ${names[department]}, can you help me with…`} required/><div><p>{!configured ? "AI isn’t connected yet. You can save a request for later." : snapshot?.paused ? "The crew is paused. Your request will wait." : "Your reply appears here when the work finishes."}</p><button className={styles.primary} disabled={busy || !snapshot || !message.trim()}>{actionable ? "Send message" : "Save for later"}<ArrowUpRight size={19}/></button></div></form></> : chatView === "assign" ? <div className={styles.assignmentBoard}><h2>Give {names[department]} a job</h2><p>Pick one. The full assignment is already written.</p>{!actionable && <p className={styles.preview}>{!configured ? "AI setup is not finished. Tasks you give now will be saved and wait for activation." : snapshot?.paused ? "The crew is paused. These tasks will wait until work resumes." : "Checking the connection. Your saved tasks stay in the work list."}</p>}<div className={styles.assignmentGrid}>{workAssignments[department].map(assignment => <article key={assignment.id}><span className={styles.eyebrow}>READY-MADE JOB</span><h3>{assignment.title}</h3><p>{assignment.outcome}</p><details><summary>See the assignment</summary><p>{assignment.instruction}</p></details><button disabled={busy || !snapshot || departmentJobs.some(j => ["queued","running"].includes(j.status) && j.message===assignmentMessage(assignment))} onClick={async () => { if(await action({operation:"message", department, message:assignmentMessage(assignment), requestKey:crypto.randomUUID()}))setChatView("work"); }}>{departmentJobs.some(j => ["queued","running"].includes(j.status) && j.message===assignmentMessage(assignment)) ? "Already assigned" : actionable ? "Give this work" : "Save this task"}<ArrowUpRight size={17}/></button></article>)}</div><button className={styles.customWork} onClick={() => setChatView("chat")}><PenLine size={17}/>Write my own assignment</button><p className={styles.muted}>Drafts and analysis stay private. Spending, publishing, and sensitive customer actions need your OK.</p></div> : <div className={styles.workScreen}><div className={styles.screenTop}><span>LIVE WORK WINDOW</span><Status value={!fresh ? "Connection needs a check" : working && agentState(department).label === "Working" ? "Working now" : agentState(department).label} tone={agentState(department).tone}/></div><div className={styles.workStage}><div className={`${styles.deskCharacter} ${styles[colors[department]]}`}><PixelCrew role={department}/></div><div><h2>{agentState(department).detail}</h2><p>{currentTask?.title || departmentJobs.find(j => j.status === "running")?.message || "No task is running right now."}</p><small>{updatedAt ? `Last checked ${new Date(updatedAt).toLocaleTimeString()}` : "Checking…"} · Updates every 5 seconds while open</small></div></div><div className={styles.processLog}>{!departmentEvents.length ? <p>No work steps recorded yet. Real updates appear here when this agent starts a request.</p> : [...departmentEvents].slice(0, 30).reverse().map(event => <article key={event.id}><time>{new Date(event.created_at).toLocaleTimeString()}</time><div><b>{eventNames[event.event] || label(event.event)}</b>{event.event === "agent_sources_checked" && <p>{event.details?.verifiedSources ?? "—"} sources checked successfully · {event.details?.unavailableSources ?? "—"} unavailable</p>}</div></article>)}</div><p className={styles.screenNote}>Follow recorded work steps here. Finished replies and drafts appear in Messages and the work list below.</p></div>}</section><section className={styles.panel}><h2>{names[department]}’s work</h2>{!snapshot?.jobs.some(j => j.department === department) && <p>No requests saved yet.</p>}{snapshot?.jobs.filter(j => j.department === department).slice(0, 6).map(j => <div className={styles.listItem} key={j.id}><div><p>{j.message}</p><small>{time(j.created_at)} · {label(j.status)}{j.error_code && " · This needs a check"}</small></div>{j.status === "queued" && <button disabled={busy} onClick={() => action({ operation: "cancel_job", id: j.id })}>Cancel</button>}</div>)}{snapshot?.tasks.filter(t => t.department === department).map(t => <details className={styles.task} key={t.id}><summary>{t.title}<small>{label(t.status)}</small></summary>{t.result && <pre className={styles.prose}>{t.result}</pre>}{["queued", "blocked"].includes(t.status) && <button disabled={busy || !actionable} onClick={() => action({ operation: "run_task", id: t.id, requestKey: crypto.randomUUID() })}>Work on this</button>}</details>)}</section>
        ...
        <HandoffFeed coordination={snapshot?.coordination} department={department} />
        </div></div></>}

    {section === "Approvals" && <><div className={styles.pageTitle}><span className={styles.eyebrow}>YOU HAVE THE FINAL SAY</span><h1>Needs your OK</h1><p>Check a suggestion. Say yes, ask for a change, or say no.</p></div><section className={styles.panel}>{!snapshot ? <p>Checking your decisions…</p> : !pending.length && <div className={styles.empty}><ShieldCheck size={42}/><h2>You’re all caught up.</h2><p>No decisions waiting for you.</p></div>}{snapshot?.approvals.map(a => <article className={styles.approval} key={a.id}><div className={styles.panelHeading}><small>{label(a.payload.kind)}</small><Status value={label(a.effective_status)}/></div><h2>{a.payload.summary}</h2>{mediaAutopilot.enabled && isPublication(a.payload) && a.effective_status==="pending" && <p>Queued for automatic posting. You can skip it, request a change, or publish it now.</p>}<pre className={styles.prose}>{a.payload.details}</pre>{hasPublicationImages(a.payload) && <div className={styles.carouselPreview}>{a.payload.assets.map((asset,i)=><figure key={asset.sha256}><a href={asset.url} target="_blank" rel="noreferrer"><Image src={asset.url} alt={asset.altText} width={1122} height={1402} unoptimized /></a><figcaption>Slide {i+1} of 3</figcaption></figure>)}</div>}<details><summary>Why this was suggested</summary><p>{a.payload.evidence.map(id => sourceNames[id] || label(id)).join(", ")}</p><small>Decision expires {time(a.expires_at)}</small></details>{a.effective_status === "pending" && <><label>Anything you’d like changed?<textarea aria-label={`Note for ${a.payload.summary}`} value={revision[a.id] || ""} onChange={e => setRevision({ ...revision, [a.id]: e.target.value })} maxLength={2000} placeholder="Optional, unless you want a change"/></label><div className={styles.buttonRow}>{([['approved', 'Yes, approve'], ['revision_requested', 'Make a change'], ['declined', 'No thanks']] as const).map(([decision, text]) => <button key={decision} disabled={busy || (decision === "revision_requested" && !revision[a.id]?.trim())} onClick={() => action({ operation: "decide", id: a.id, payloadHash: a.payload_hash, decision, note: revision[a.id] || "Owner Command Center decision" })}>{decision === "approved" && isPublication(a.payload) ? `${mediaAutopilot.enabled ? "Publish now on" : "Approve & publish on"} ${publicationNetwork(a.payload)}` : text}</button>)}</div>{isPublication(a.payload) && <p className={styles.muted}>Approving publishes {isInstagramPublication(a.payload) ? "these exact slides and caption" : "the exact text above"} publicly on the displayed {publicationNetwork(a.payload)} account now.</p>}</>}{isPublication(a.payload) && a.effective_status === "approved" && <div><p>{a.delivery?.published ? "Published · confirmed by Buffer" : a.delivery ? `Delivery: ${label(a.delivery.state)}. Publication is not confirmed.` : "Approved · publishing has not started."}</p>{a.delivery?.postId && <small>Buffer receipt: {a.delivery.postId}</small>}<div className={styles.buttonRow}><button disabled={busy || (!a.delivery && new Date(a.expires_at).getTime() <= clock)} onClick={() => action({ operation: isDailySocialPayload(a.payload) ? "social_receipt" : isInstagramPublication(a.payload) ? (a.delivery ? "instagram_receipt" : "instagram_publish_approved") : (a.delivery ? "buffer_receipt" : "buffer_publish_approved"), id: a.id, payloadHash: a.payload_hash })}>{isDailySocialPayload(a.payload) || a.delivery ? "Check receipt" : "Send approved post"}</button></div>{a.delivery?.state === "unknown" && <p>Check Buffer and {publicationNetwork(a.payload)} directly. This post cannot be submitted again.</p>}</div>}</article>)}<p className={styles.muted}>Routine media posts publish automatically under your standing instruction. Other proposals record a planning decision only.</p></section></>}

    {section === "Results" && <><div className={styles.pageTitle}><span className={styles.eyebrow}>REAL NUMBERS. PLAIN ENGLISH.</span><h1>How’s business?</h1><p>{ready ? `Last checked ${time(ready.checkedAt)}` : "Checking your connected numbers…"}</p><button disabled={busy} onClick={checkConnections}>Update numbers</button></div><div className={styles.metricGrid}><Metric caption="USD payments · recent 30-day group" value={paymentTotals ? `$${((usdPayments?.current ?? 0)/100).toFixed(2)}` : "—"} detail={`Successful payments created in the last 30 days, before refunds and fees. Previous 30-day group: ${paymentTotals ? `$${((usdPayments?.previous ?? 0)/100).toFixed(2)}` : "not verified"}. Whole account; not profit.`}/><Metric caption="Subscriptions ended · 30 days" value={subscriptions?.endedLast30Days ?? "—"} detail={`${subscriptions?.scheduledToCancel ?? "—"} active or trialing subscriptions scheduled to cancel. These are subscription counts, not a customer churn rate.`}/><Metric caption="Active subscriptions" value={subscriptions?.complete ? subscriptions.counts?.active ?? 0 : "—"} detail="From your whole Stripe account. Subscriptions, not unique customers."/><Metric caption="Website link clicks" value={clicks ?? "—"} detail={`Last 30 days. Clicks do not mean purchases.${ready?.environment === "preview" ? " From the preview database copy." : ""}`}/><Metric caption="Campaign-tagged paid checkouts" value={taggedPaid ?? "—"} detail={`Initial paid checkouts, not unique customers or a click conversion rate. ${conversions ? `${taggedTrials} tagged trial starts; ${conversions.untaggedSessions} sessions without campaign tags.` : "Stripe checkout tracking has not been verified yet."}`}/></div>{ready?.environment === "preview" && <p className={styles.preview}>Stripe is checked live. Website and partner records come from a database copy while this is in preview.</p>}<section className={styles.panel}><h2>Payments and cancellation checks</h2><p>Payment totals are grouped by currency and compared by when the payment was created. They are before refunds, fees and disputes—not profit. Subscription details show endings in the last 30 days and scheduled cancellations.</p>{paymentData?.status !== "verified" && <p>Payment totals could not be verified yet.</p>}<h2>Where the numbers come from</h2>{ready?.evidence.map(e => <details className={styles.source} key={e.id}><summary><span>{sourceNames[e.id] || label(e.id)}</span><Status value={e.status === "verified" ? "Connected" : "Not connected"} tone={e.status === "verified" ? "green" : "amber"}/></summary><p>{e.scope}</p>{e.status === "verified" && <DataView data={e.data}/>}</details>)}</section></>}

    {section === "Settings" && <><div className={styles.pageTitle}><span className={styles.eyebrow}>THE LITTLE EXTRAS</span><h1>Settings & help</h1><p>Everything you need less often lives here.</p></div><InstallApp /><BufferPanel connection={ready?.buffer} busy={busy} storageReady={ready?.store.tablesPresent === true} onCheck={checkConnections} onTest={() => action({ operation: "buffer_draft_test" })} onPrepare={text => action({ operation: "buffer_prepare", text })} /><section className={styles.panel}><h2>Your crew’s setup</h2><p><Link href="/owner/connections">Connect social research and view Indicator Lab connections →</Link></p><p>Every agent needs the app’s AI connection and business tools. Account access from this chat is not automatically shared with the crew.</p><div className={styles.listItem}><span>X publishing</span><Status value={ready?.buffer?.ready ? "Automatic routine posting" : "Check connection"} tone={ready?.buffer?.ready ? "quiet" : "amber"}/></div><div className={styles.listItem}><span>AI connection</span><Status value={!ready ? "Checking" : tested ? "Verified by completed work" : configured ? "Configured · needs a real run" : ready.ai.credentialPresent ? "Key saved · routine work off" : "Needs setup"} tone={configured ? "quiet" : "amber"}/></div><div className={styles.listItem}><span>Automatic daily work</span><Status value={!ready ? "Checking" : ready.configuration.autonomyEnabled ? "Switched on" : "Not switched on"}/></div><div className={styles.listItem}><span>Private Telegram</span><Status value={!ready ? "Checking" : ready.configuration.privateTelegramEnabled ? "Switched on · check connection" : "Needs setup"}/></div><div className={styles.buttonRow}><button disabled={busy || !snapshot || (!configured && snapshot.paused)} onClick={() => action({ operation: "pause", paused: !snapshot?.paused })}>{snapshot?.paused ? <Play size={17}/> : <Pause size={17}/>} {snapshot?.paused ? "Resume crew" : "Pause crew"}</button><button onClick={() => { void refresh(); void checkConnections(); }}>Check setup again</button></div><p className={styles.muted}>Pausing stops new work. Any request already sent to an outside service may finish.</p></section><details className={styles.panel}><summary>Work history</summary>{!snapshot?.activity.length && <p>No activity recorded yet.</p>}{snapshot?.activity.map(a => <div className={styles.listItem} key={a.id}><div><p>{label(a.event)}</p><small>{names[a.actor as Department] || label(a.actor)} · {time(a.created_at)}</small></div></div>)}</details><details className={styles.panel}><summary>Advanced setup</summary><p>{ready?.dataScope}</p><div className={styles.buttonRow}><button disabled={busy || ready?.store.tablesPresent !== false} onClick={() => action({ operation: "initialize" })}>Set up work storage</button><button disabled={busy || !actionable} onClick={() => action({ operation: "work" })}>Start next waiting request</button><button disabled={busy || !snapshot} onClick={() => action({ operation: "telegram_check" })}>Check Telegram</button><button disabled={busy || !ready?.configuration.privateTelegramEnabled} onClick={() => action({ operation: "telegram_connect" })}>Connect private bot</button><button disabled={busy || !ready?.configuration.privateTelegramEnabled} onClick={() => action({ operation: "deliver" })}>Send waiting owner notices</button></div>{ready && Object.entries(ready.configuration).map(([key, value]) => <div className={styles.listItem} key={key}><span>{label(key.replace(/([A-Z])/g, " $1"))}</span><Status value={value ? "Set" : "Not set"}/></div>)}{snapshot?.outbox.filter(x => x.status === "unknown").map(x => <p key={x.id}>An owner notice has unconfirmed delivery. Check Telegram before trying again.</p>)}</details><section className={styles.panel}><h2>This is your private space</h2><p>Your customer Telegram community stays separate. This device remembers you until you disconnect or its session expires.</p><button onClick={logout}><LockKeyhole size={17}/>Disconnect this device</button></section></>}
...
function CoordinationPanel({ coordination, now, fresh, paused }: {
    coordination?: Coordination;
    now: number;
    fresh: boolean;
    paused: boolean;
}) {
    const heartbeat = coordination?.heartbeat;
    const heartbeatAge = heartbeat ? now - new Date(heartbeat.lastSeenAt).getTime() : NaN;
    const heartbeatFresh = fresh && Number.isFinite(heartbeatAge) && heartbeatAge >= 0 && heartbeatAge <= 120000;
    const workerHealthy = heartbeatFresh && ["idle", "working"].includes(heartbeat?.status || "");
    const running = Boolean(coordination?.enabled && workerHealthy && !paused);
    const status = !coordination ? "Setup pending" : !coordination.enabled ? "Automatic handoffs off" : paused ? "Crew paused" : !fresh ? "Dashboard needs refresh" : !heartbeat ? "Waiting for worker check-in" : !heartbeatFresh ? "Worker check-in overdue" : workerHealthy ? heartbeat?.status === "idle" ? "Worker ready · idle" : "Worker connected" : `Worker: ${label(heartbeat?.status || "unknown")}`;
    const budget = coordination?.budget;
    const dollarLimit = (value: number | undefined) => typeof value === "number" && Number.isFinite(value) ? `$${value.toFixed(2)}` : "Not set";
    return <section className={`${styles.panel} ${styles.coordination}`} aria-label="Automatic agent coordination">
        <div className={styles.panelHeading}><div><span className={styles.eyebrow}>WORKING TOGETHER</span><h2>Agent handoffs</h2></div><Status value={status} tone={running ? "green" : "amber"} /></div>
        <p>{running ? "The worker checks for new work and passes saved assignments between agents. An idle worker is ready for the next job." : "Automatic teamwork needs an enabled worker with a recent check-in. Saved assignments stay visible here while setup or a connection needs attention."}</p>
        <div className={styles.coordinationStats}>
            <div><span>Waiting handoffs</span><strong>{coordination?.pendingHandoffs ?? "—"}</strong></div>
            <div><span>Completed handoffs</span><strong>{coordination?.completedHandoffs ?? "—"}</strong></div>
            <div><span>AI work allowance</span><strong className={styles.allowance}>{budget?.configured ? `${dollarLimit(budget.dailyLimitUsd)} / day` : "Setup pending"}</strong><small>{budget?.configured ? `${dollarLimit(budget.monthlyLimitUsd)} / month` : "No routine spending limit confirmed"}</small></div>
        </div>
        <small>{heartbeat ? `Last worker check-in: ${time(heartbeat.lastSeenAt)} · ${label(heartbeat.status)}` : "No worker heartbeat recorded yet."}{!fresh && coordination ? " · Dashboard data needs refreshing." : ""}</small>
        <HandoffFeed coordination={coordination} compact />
    </section>;
}
function HandoffFeed({ coordination, department, compact = false }: {
    coordination?: Coordination;
    department?: Department;
    compact?: boolean;
}) {
    const handoffs = (coordination?.recentHandoffs || []).filter(item => !department || item.from_department === department || item.to_department === department).slice(0, compact ? 4 : 8);
    return <section className={compact ? styles.handoffFeed : `${styles.panel} ${styles.handoffFeed}`} aria-label={department ? `${names[department]} handoffs` : "Recent agent handoffs"}>
        <h3>{department ? `${names[department]}’s team handoffs` : "Recent handoffs"}</h3>
        {handoffs.length ? <ul>{handoffs.map(item => <li key={item.id}><div><b>{names[item.from_department as Department] || label(item.from_department)} → {names[item.to_department as Department] || label(item.to_department)}</b><time dateTime={item.created_at}>{time(item.created_at)}</time></div><Status value={label(item.status)} tone={item.status === "completed" ? "green" : "quiet"} /></li>)}</ul> : <p>{coordination ? "No handoffs recorded here yet. Agent-to-agent assignments appear as they are saved." : "Team handoffs will appear when coordination is connected."}</p>}
    </section>;
}
