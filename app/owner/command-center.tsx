...
export default function CommandCenter(){
  ...
  async function login(event:FormEvent<HTMLFormElement>){
    ...
    try{const response=await fetch("/api/owner/session",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({key})});const data=await response.json();if(!response.ok)throw new Error(data.error);form.reset();setAuth(true);}catch(e){setError((e as Error).message);}finally{setBusy(false);}
  }
  ...
  if(!auth)return <main id="main-content" className={styles.login}><div className={styles.loginCard}><div className={styles.eyebrow}><LockKeyhole size={16}/> PRIVATE OWNER ACCESS</div><h1>DARTH ALGO<span>Command Center</span></h1><p>Your agents, business evidence, and decisions in one private workspace.</p><form onSubmit={login}><label htmlFor="owner-key">Owner access key</label><input id="owner-key" name="key" type="password" autoComplete="current-password" minLength={32} maxLength={256} required placeholder="Enter your private owner key"/><button disabled={busy || !enabled}>{busy ? "Signing in…" : "Open Command Center"}<ArrowUpRight size={17}/></button></form>{!enabled && <p className={styles.muted}>Owner access is disabled on this deployment. Preview configuration must be completed before sign-in.</p>}{error && <p role="alert" className={styles.error}>{error}</p>}<small>Owner sessions expire after eight hours. This is separate from the customer Telegram community.</small></div></main>;
