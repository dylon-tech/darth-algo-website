...
export default function CommandCenter() {
    ...
    const liveTitle = !snapshot || !ready ? "Checking on your crew…" : !fresh ? "Let’s check the connection." : !configured ? "Your crew needs its AI connection." : snapshot.paused ? "Your crew is taking a break." : working ? `${working} ${working === 1 ? "agent is" : "agents are"} working.` : "Your crew is waiting for work.";
    const summary = !ready ? "Getting the latest status." : !configured ? "All 8 roles are built. They can start thinking and replying once AI setup is complete." : snapshot?.paused ? "Resume when you’re ready for work to continue." : ready.configuration.autonomyEnabled ? "Daily work is enabled. The agents run when a request or scheduled check starts." : "Agents can handle requests. Automatic daily work is not switched on yet.";
