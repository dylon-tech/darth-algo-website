...
const initialFindings=[
  {track:"Product",finding:"A clean opening-range reference is a useful exploratory draft. Public authors describe session high, low and midpoint displays and an OR/VWAP retest workflow. These are author descriptions, not measured demand or verified results.",hypothesis:"An uncluttered free session reference may help futures traders read context before using Darth Algo's paid tools.",confidence:"Low · two independent public author descriptions; customer demand unmeasured",sources:["https://www.tradingview.com/script/l5UakOY7-Opening-Range-basic/","https://www.tradingview.com/script/gBh2TZH5-ORB-VWAP-NY-Bounce/"]},
  {track:"Growth",finding:"A competitor publicly presents a free starting option beside feature comparisons and paid plans. This documents positioning, not the competitor's conversion rate.",hypothesis:"A clearly explained free companion library may help qualified visitors understand the broader Darth Algo catalog.",confidence:"Low · competitor self-description; no Darth Algo conversion test",sources:["https://www.luxalgo.com/pricing/"]}
];
...
export default async function ResearchPage(){
  ...
  return <main className="min-h-screen bg-[#08090c] px-5 py-10 text-white"><div className="mx-auto max-w-4xl space-y-8">
    ...
    <section className="space-y-4"><h2 className="text-2xl font-semibold">Initial sourced brief · September 23, 2026</h2><p className="text-sm text-zinc-400">Manually reviewed public pages. Publication dates and engagement measures are unknown unless stated on the linked page. These hypotheses do not authorize a release, a price change or a performance claim.</p>{initialFindings.map(f=><article key={f.track} className="rounded-2xl border border-white/10 bg-[#11141b] p-5"><h3 className="font-semibold text-violet-300">{f.track}</h3><p className="mt-2 text-sm text-zinc-200">{f.finding}</p><p className="mt-2 text-sm text-zinc-400">Hypothesis: {f.hypothesis}</p><p className="mt-2 text-xs text-amber-300">{f.confidence}</p><ul className="mt-3 space-y-1 text-xs">{f.sources.map(url=><li key={url}><a href={url} target="_blank" rel="noopener noreferrer" className="break-all text-violet-300 underline">{url}</a></li>)}</ul></article>)}</section>
