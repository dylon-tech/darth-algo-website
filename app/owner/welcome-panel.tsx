...
export default function WelcomePanel({compact=false}:{compact?:boolean}){
 ...
 return <section className={styles.card} aria-label='Welcome Agent'><header className={styles.header}><Image src='/founder/darth-algo-color.jpg' alt='Darth Algo' width={48} height={48} unoptimized/><div><span className={styles.kicker}>GROWTH</span><h2>Welcome Agent</h2></div><span className={styles.status}>Needs setup</span></header>
 ...
 <div className={styles.callout}><strong>WELCOME needs one correction</strong><p>The current 25% coupon permits only one redemption total. Offers stay off while the replacement terms and checkout tests are reviewed.</p></div>
 ...
 <div className={styles.actions}><button disabled={busy} onClick={()=>void action(p.platform,'pause')}>Pause</button><button disabled={busy} onClick={()=>void action(p.platform,'resume')}>Resume</button><button disabled={busy} onClick={()=>void action(p.platform,'test')}>Send Test</button><button onClick={()=>{setEditing(p.platform);setDraft(p.message);}}>Edit Message</button></div>
