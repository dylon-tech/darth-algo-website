...
export function ReviewActions({id,sourceHash,stage,approvalId,payloadHash}:{id:string;sourceHash:string;stage:string;approvalId?:string;payloadHash?:string}){
  ...
  async function decide(decision:"approved"|"declined"|"revision_requested"){
    if(!approvalId||!payloadHash)return;
    setBusy(true);setResult("");
    try{const response=await fetch("/api/owner/command",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({operation:"decide",id:approvalId,payloadHash,decision,note})});
      const body=await response.json();if(!response.ok)throw Error(body.error||"Decision not saved");
      setResult(decision==="approved"?"Exact package approved for supervised free release. Nothing has been published yet.":"Decision saved. Reload to see its new stage.");
    }catch(error){setResult(error instanceof Error?error.message:"Decision not saved");}finally{setBusy(false);}
  }
  return <section className="rounded-2xl border border-white/10 p-5"><h2 className="text-xl font-semibold">Your review</h2>{stage==="qa_blocked"?<><p className="mt-2 text-sm text-amber-300">TradingView compilation and exact-version chart preview are pending. Release approval is unavailable.</p><label className="mt-4 block text-sm" htmlFor="revision-note">Compiler error, change request or paid proposal rationale</label><textarea id="revision-note" value={note} onChange={e=>setNote(e.target.value)} maxLength={2000} className="mt-2 h-28 w-full rounded-lg bg-black p-3 text-white"/><div className="mt-3 flex flex-wrap gap-3"><button disabled={busy||!note.trim()} onClick={()=>void act("test_feedback")} className="rounded-lg border border-white/20 px-4 py-2 disabled:opacity-40">Send test error or notes</button><button disabled={busy||!note.trim()} onClick={()=>void act("request_changes")} className="rounded-lg border border-white/20 px-4 py-2 disabled:opacity-40">Request Changes</button><button disabled={busy} onClick={()=>void act("reject")} className="rounded-lg border border-red-500/40 px-4 py-2 text-red-300 disabled:opacity-40">Reject</button><button disabled={busy||!note.trim()} onClick={()=>void act("make_paid")} className="rounded-lg border border-white/20 px-4 py-2 disabled:opacity-40">Make Paid proposal</button></div></>:stage==="pending"&&approvalId&&payloadHash?<><p className="mt-2 text-sm text-amber-300">This approves the exact package for assisted free release. It does not publish to TradingView.</p><textarea aria-label="Optional decision note" value={note} onChange={e=>setNote(e.target.value)} maxLength={2000} className="mt-4 h-24 w-full rounded-lg bg-black p-3"/><div className="mt-3 flex flex-wrap gap-3"><button disabled={busy} onClick={()=>void decide("approved")} className="rounded-lg bg-violet-300 px-4 py-2 font-semibold text-black disabled:opacity-40">Approve for assisted free release</button><button disabled={busy||!note.trim()} onClick={()=>void decide("revision_requested")} className="rounded-lg border border-white/20 px-4 py-2 disabled:opacity-40">Request Changes</button><button disabled={busy} onClick={()=>void decide("declined")} className="rounded-lg border border-red-500/40 px-4 py-2 text-red-300 disabled:opacity-40">Reject</button></div></>:<p className="mt-2 text-zinc-400">Current stage: {stage}. Exact-version approval is available only after private test and education package checks.</p>}<p role="status" className="mt-3 text-sm text-violet-300">{result}</p></section>;
}
