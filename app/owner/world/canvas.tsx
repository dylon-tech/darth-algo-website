...
export default function Campus({agents,data,fresh,onRoom,onAgent,focus}:{agents:WorldAgent[];data:CeoHome|null;fresh:boolean;onRoom:(room:RoomId)=>void;onAgent:(id:string,mode?:AgentMode)=>void;focus:RoomId|null}){
 const [watch,setWatch]=useState<RoomId>('content'),[overview,setOverview]=useState(false),[zoom,setZoom]=useState(false),[hidden,setHidden]=useState(false),[motion,setMotion]=useState(true);
 ...
 useEffect(()=>{const active=agents.find(a=>a.active);if(active&&!chosen.current){setWatch(active.room);chosen.current=true;}},[agents]);
 ...
 const choose=(id:RoomId)=>{chosen.current=true;setWatch(id);setOverview(false);setZoom(false);};
