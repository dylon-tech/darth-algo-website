...
export default function World(){
 ...
 return <main id='main-content' className={styles.world}>
  <div inert={modal}>
   ...
   <div className={styles.heading}><div><p className={styles.eyebrow}>COMMAND CAMPUS / 01</p><h1 id='hq-heading' tabIndex={-1}>Your business. Your crew<span>.</span></h1><p>Watch a department, inspect its work, or step in to help.</p></div><button className={styles.refresh} disabled={loading||offline} onClick={()=>void refresh(true)} aria-label='Refresh headquarters'><RefreshCw size={17}/>{loading?'Checking…':'Refresh'}</button></div>
   ...
   <div className={styles.campusLayout}><div><Canvas agents={state?.agents||[]} data={data} fresh={fresh} onRoom={id=>navigate(id)} onAgent={(id,mode)=>openAgent(id,mode)} focus={room}/></div>
  ...
  {modal&&<IphoneSheet onClose={close}>{selectedAgent&&data?<AgentPanel key={selectedAgent.id} agent={selectedAgent} mode={mode} onMode={setMode} fresh={fresh} now={clock} issues={data.issues} onClose={close} onChanged={()=>void refresh(true)} worldStatus={state?.agents.find(a=>a.id===selectedAgent.id)?.status} preview={selectedAgent.id==='content'&&data.queue[0]?.image?{image:data.queue[0].image,text:'Next saved campaign artwork · not a live generation'}:undefined}/>:<section className={styles.panel} role='dialog' aria-modal='true' aria-labelledby='world-panel-title'>
