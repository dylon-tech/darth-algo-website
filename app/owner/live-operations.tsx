  ...
  return <main id='top' className={styles.shell}>
    <div className={styles.wrap}>
      ...
      <section id='publication' className={styles.card}>
        ...
        {snapshot.content.previewUrl&&<div className={styles.preview}><Image src={snapshot.content.previewUrl} alt='First slide of the actual saved Darth Algo daily campaign' width={1080} height={1350} unoptimized className={styles.previewImage}/><div><span className={styles.eyebrow}>CURRENT SAVED CAMPAIGN</span><h3>{snapshot.content.assets} prepared slides</h3><p>{snapshot.content.caption||'Caption available in existing controls.'}</p><span className={styles.small}>Prepared {stamp(snapshot.content.preparedAt)}</span></div></div>}
        ...
        <p className={styles.note}>Posts are scheduled for 9 AM and 3 PM Eastern. Connection or uncertain prior receipts may delay a slot. Prepared is not published. Whop targets Home, not Chats.</p>
      </section>
