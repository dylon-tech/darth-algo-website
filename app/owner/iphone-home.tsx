...
import {Activity,ArrowUpRight,Bot,ChevronRight,Ellipsis,House,Inbox,Plus,RefreshCw,Send,Wallet,Wrench,X} from 'lucide-react';
 ...
 return <main className={styles.app} id='home'>
  <div className={styles.content} inert={modal}>
   ...
   <section hidden={view!=='bills'} className={styles.screen} aria-label='Company finances'>
    <div className={styles.stats}><div><span>Current customers</span><strong>{f?.customers.active??'—'}</strong><small>{f?.customers.trials??'—'} trials</small></div><div><span>Bills / income</span><strong>{f?.expenses.ratio!=null?`${f.expenses.ratio.toFixed(1)}%`:'—'}</strong><small>{f?.expenses.complete?'Monthly costs':'Estimated / incomplete costs'}</small></div></div>
    ...
    <details className={styles.groupDetails}><summary>Sources & estimates <ChevronRight size={17}/></summary><div><p>{f?.income.scope||'Financial records could not be read.'} {f?.customers.scope}</p><p>Monthly bills are equivalent budgets, including estimates. Annual bills are divided by 12. Bills / income is operating costs divided by 30-day income, not net profit or personal debt-to-income.</p>{Boolean(f?.income.otherCurrencies.length)&&<p>Excluded currencies: {f?.income.otherCurrencies.join(', ').toUpperCase()}. No currency conversion is applied.</p>}<p>Income checked {time(f?.income.checkedAt)}. Customers checked {time(f?.customers.checkedAt)}. {f?.billSync}</p></div></details>
   </section>
  </div>
  <nav className={styles.nav} aria-label='CEO navigation' inert={modal}>{([{id:'home',label:'HQ',Icon:House},{id:'team',label:'Agents',Icon:Bot},{id:'queue',label:'Queue',Icon:Send},{id:'bills',label:'Money',Icon:Wallet}] as const).map(({id,label,Icon})=><button key={id} aria-current={view===id?'page':undefined} onClick={()=>navigate(id)}><span><Icon size={22}/></span>{label}</button>)}</nav>
