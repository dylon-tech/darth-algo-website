...
import {Activity,ArrowUpRight,Bot,ChevronRight,Ellipsis,FlaskConical,House,Inbox,Pause,Play,Plus,RefreshCw,Send,Wallet,Wrench,X} from 'lucide-react';
...
export default function IphoneHome({workspace}:{workspace:ReactNode}){
 ...
 async function act(operation:'bill'|'recover'|'suggestion'|'pause',body:Record<string,unknown>={}){
  ...
  try{const r=await fetch(operation==='pause'?'/api/owner/command':'/api/owner/dashboard',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},signal:c.signal,body:JSON.stringify({operation,...body})});const result=await r.json();if(!r.ok)throw Error(result.error||'The change was not confirmed.');if(operation==='pause'&&result.paused!==body.paused)throw Error('The pause state was not confirmed. Refresh before retrying.');if(mounted.current){setNotice(operation==='pause'?(result.paused?'Automation paused. Calls already in progress may finish.':'Automation resumed. Saved work remains in its queue.'):result.message||'Saved.');if(operation==='bill')setBill(null);await refresh();}}
