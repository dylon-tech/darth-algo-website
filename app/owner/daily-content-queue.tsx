...
export default function DailyContentQueue({active,disabled,onOpenAgent}:{active:boolean;disabled:boolean;onOpenAgent:()=>void}){
 ...
 return <section className={styles.queue} aria-labelledby='daily-queue-title'>
  ...
  <div className={styles.queueGrid}>{items.map(item=><article key={item.id} className={styles.queueCard}>
   ...
   {item.revision?.brief&&<details className={styles.details}><summary>View replacement brief</summary><p className={styles.pre}>{item.revision.brief}</p><p>Internal copy and artwork instructions—not a rendered replacement. The slot stays held until new artwork is reviewed.</p></details>}
   ...
   {editing===item.id&&<form className={styles.remakeForm} onSubmit={e=>{e.preventDefault();void decide(item,'remake');}}><label htmlFor={`note-${item.id}`}>What should change? <small>Optional</small></label><textarea id={`note-${item.id}`} rows={2} maxLength={800} value={note} onChange={e=>setNote(e.target.value)} placeholder='Stronger hook, different visual, clearer feature…'/><p>The original stops posting. The Content agent prepares replacement copy and artwork instructions; new images still require visual review.</p><div><button type='button' onClick={()=>setEditing(null)}>Cancel</button><button type='submit' disabled={unavailable||Boolean(busy)}>Request remake</button></div></form>}
  ...
  <p className={styles.help}>Disapprove holds the slot. Remake creates a real agent request, not an instant image. Repeated taps cannot create duplicate requests.</p>
 </section>;
}
