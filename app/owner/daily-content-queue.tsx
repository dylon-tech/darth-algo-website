...
export default function DailyContentQueue({active,disabled,onOpenAgent}:{active:boolean;disabled:boolean;onOpenAgent:()=>void}){
 const [data,setData]=useState<ContentQueue|null>(null),[error,setError]=useState(''),[notice,setNotice]=useState(''),[busy,setBusy]=useState(''),[loading,setLoading]=useState(false),[filter,setFilter]=useState<'today'|'upcoming'|'held'|'recent'>('today'),[editing,setEditing]=useState<string|null>(null),[note,setNote]=useState('');
 ...
 async function decide(item:QueueItem,action:ContentAction){
  if(locked.current||unavailable||(item.submitted&&action!=='disapprove'))return;locked.current=true;setBusy(item.id);setNotice('');
 ...
 const items=data?.items.filter(c=>filter==='today'?c.day===data.today:filter==='upcoming'?c.day>data.today:filter==='recent'?c.day<data.today:Boolean(c.decision&&(c.decision.action!=='approve_replacement'||c.decision.reviewHash!==c.reviewHash)))||[];
 return <section className={styles.queue} aria-labelledby='daily-queue-title'>
  ...
  <div className={styles.filters} aria-label='Filter content queue'>{(['today','upcoming','held','recent'] as const).map(f=><button key={f} aria-pressed={filter===f} onClick={()=>setFilter(f)}>{f==='today'?'Today':f==='upcoming'?'Upcoming':f==='recent'?'Recent':'Held'}</button>)}</div>
  ...
  <div className={styles.queueGrid}>{items.map(item=><article key={item.id} className={styles.queueCard}>
   ...
   {item.deliveries&&<details className={styles.details} open><summary>Delivery by destination</summary>{item.deliveries.map(d=><div key={d.network}><strong>{d.network.toUpperCase()}</strong><p>{d.state}</p>{d.url&&<a href={d.url} target='_blank' rel='noreferrer'>View published post</a>}{d.postId&&<small>Receipt {d.postId}</small>}{d.checkedAt&&<small>Checked {new Date(d.checkedAt).toLocaleString()}</small>}</div>)}</details>}
   ...
   {<div className={styles.queueActions}><button disabled={unavailable||Boolean(busy)||item.decision?.action==='disapprove'} onClick={()=>void decide(item,'disapprove')}><X size={16}/>{busy===item.id?'Saving…':'Disapprove'}</button><button disabled={item.submitted||unavailable||Boolean(busy)||Boolean(item.revision&&['queued','running','unknown'].includes(item.revision.status))} onClick={()=>{setEditing(editing===item.id?null:item.id);setNote('');}}><PenLine size={16}/>Remake</button></div>}
   ...
   {item.submitted&&<p className={styles.help}>Disapprove stops destinations that have not started. Already dispatched or published posts stay recorded above.</p>}
   </div>
  </article>)}</div>{!items.length&&<p className={styles.empty}>{loading&&!data?'Loading the real publishing queue…':filter==='today'?'No content is recorded for today.':filter==='held'?'No held content in the returned queue.':filter==='recent'?'No recent content in the returned queue.':'No upcoming content in the returned queue.'}</p>}
