...
const productComparison = [
  ...
  ["Pro", "Scalper + Swing together", "$29/mo", "Best monthly bundle"],
  ["Lifetime", "Full suite ownership", "$134.99", "Source code included"],
];
...
export default function Home() {
  ...
  return (
    <main id="main-content" className="app-shell relative isolate min-h-screen bg-obsidian text-white">
      ...
      <section className="relative z-10 border-b border-white/10 bg-black/45 py-10">
        <div className="section-shell">
          <div className="grid gap-px overflow-hidden rounded-md border border-white/10 bg-white/10 sm:grid-cols-2 lg:grid-cols-4">
            {trustSignals.map(([Icon, title, copy]) => {
              const TrustIcon = Icon as typeof ShieldCheck;
              return (
                <article key={String(title)} className="group bg-[#080809] p-5 transition hover:bg-ember/[0.045]">
                  <div className="flex items-center gap-3">
                    <span className="grid h-10 w-10 place-items-center rounded-md border border-ember/25 bg-ember/[0.08]"><TrustIcon className="h-4 w-4 text-ember" /></span>
                    <h2 className="font-display text-base font-black text-white">{String(title)}</h2>
                  </div>
                  <p className="mt-3 text-sm leading-6 text-zinc-500">{String(copy)}</p>
                </article>
              );
            })}
          </div>
        ...
        <div className="section-shell">
          <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
            <SectionHeading
              ...
              title="A premium signal suite built like trading software."
              copy="Inspired by the cleanest trading platforms: fewer distractions, stronger visual hierarchy, faster setup recognition, and product pages that make choosing access simple."
            />
        ...
        <div className="section-shell">
          <div className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:items-end">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                ["87%", "Win rate", "87 of 100 sample trades"],
                ["1:2.3", "Avg. risk / reward", "Measured in R"],
                ["3.4", "Profit factor", "Sample gross profit / loss"],
        ...
        <div className="section-shell">
          ...
          <div className="pricing-command mx-auto mt-10 max-w-6xl overflow-hidden rounded-md border border-white/10 bg-black/70">
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 px-5 py-4">
              <div>
                <p className="font-mono text-[10px] font-black uppercase text-ember">Plan finder</p>
                <h3 className="mt-1 font-display text-2xl font-black text-white">Pick the access path that matches how you trade.</h3>
              </div>
          ...
          <div className="mx-auto mt-5 max-w-6xl overflow-x-auto rounded-md border border-white/10 bg-black/55">
            <div className="min-w-[720px]">
              <div className="grid grid-cols-[0.7fr_1fr_0.7fr_1fr] border-b border-white/10 bg-white/[0.035] px-4 py-3 font-mono text-[9px] font-bold uppercase text-zinc-600">
                <span>Plan</span><span>Best for</span><span>Price</span><span>Why pick it</span>
              </div>
              {productComparison.map(([plan, bestFor, price, reason]) => (
                <div key={plan} className="grid grid-cols-[0.7fr_1fr_0.7fr_1fr] gap-3 border-b border-white/10 px-4 py-4 text-sm last:border-b-0">
                  <strong className="font-display text-white">{plan}</strong>
                  <span className="text-zinc-400">{bestFor}</span>
                  <span className="font-bold text-ember">{price}</span>
      ...
      <section className="relative z-10 border-b border-white/10 bg-[#050506] py-16 sm:py-20">
        <div className="section-shell">
          <div className="grid gap-8 lg:grid-cols-[0.68fr_1.32fr] lg:items-center">
            <div>
              <p className="font-display text-xs font-black uppercase text-ember">Why this is different</p>
              <h2 className="mt-4 text-balance font-display text-4xl font-black leading-tight text-white sm:text-5xl">Built to feel like a trading terminal, not a basic indicator page.</h2>
              <p className="mt-5 text-base leading-7 text-zinc-400">The site now sells the full experience: cleaner charts, obvious plan choices, real screenshots, risk language, and a faster phone path from interest to checkout.</p>
            </div>
            <div className="terminal-stack grid gap-4 md:grid-cols-3">
              {[
                ["01", "Clarity layer", "Signals, trend, and trade zones are shown as one readable system."],
                ["02", "Decision path", "Visitors can quickly choose Scalper, Swing, Pro, or Lifetime."],
                ["03", "Conversion path", "Trial, pricing, username help, and Stripe links stay easy to reach."],
              ].map(([number, title, copy]) => (
                <article key={number} className="terminal-card rounded-md border border-white/10 bg-black p-5">
                  <span className="font-mono text-[10px] font-black uppercase text-ember">{number}</span>
                  <h3 className="mt-8 font-display text-2xl font-black text-white">{title}</h3>
                  <p className="mt-3 text-sm leading-6 text-zinc-500">{copy}</p>
                </article>
              ))}
            </div>
