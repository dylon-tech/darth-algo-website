...
import { proCandles as candles, proSignals, proSimulation, proCandleTick, proFocusFrame } from "./pro-chart-simulation";
...
export function createProChartWorld(host: HTMLElement): ChartWorld {
  ...
  element("text", { x: 16, y: 24, fill: "#b6bac4", "font-size": 13 }, svg, "MGC · Micro Gold Futures · 1m · COMEX");
  ...
  const render = (progress: number, demoProgress = 1, still = false) => {
    ...
    chrome.forEach(node=>show(node,feature===0));
    timeLabels.forEach((node,i)=>{
      const minutes = 9*60 + (Math.floor(scroll/122)+i)*10;
    ...
    show(plot,feature!==4); show(dashboard,feature===0||feature===4);
    show(priceBox,feature===0); show(priceText,feature===0); show(priceLine,feature!==4);
    ...
    const destination = proFocusFrame(feature,host.clientWidth<640,X(index)-scroll,Y(price),X(signal.index)-scroll,Y(signal.entry),Math.min(Y(signal.stop),Y(signal.tp2)));
