...
type AffiliateStripeObject={discounts?:Array<string|DiscountLike>;amount_paid?:number;amount_total?:number;amount_refunded?:number;customer?:string|{id:string}|null;payment_intent?:string|{id:string}|null;charge?:string|{id:string}|null;currency?:string;mode?:string};
...
export async function POST(request:Request){
  ...
  if((event.type==='checkout.session.completed'&&(event.data.object as Stripe.Checkout.Session).mode==='payment')||event.type==='invoice.paid'){
    const object=event.data.object as unknown as AffiliateStripeObject;const promoId=promotionId(object.discounts);
    if(promoId){const[affiliate]=await db()`select id,commission_rate_bps from affiliate_applications where stripe_promotion_code_id=${promoId} and status='approved'`;if(affiliate){const gross=event.type==='invoice.paid'?(object.amount_paid??0):(object.amount_total??0);const commission=Math.round(gross*affiliate.commission_rate_bps/10000);await db()`insert into affiliate_commissions(id,affiliate_id,stripe_event_id,stripe_customer_id,stripe_payment_id,gross_cents,commission_cents,currency,payable_at) values(${randomUUID()},${affiliate.id},${event.id},${idOf(object.customer)},${idOf(object.payment_intent??object.charge)},${gross},${commission},${object.currency??'usd'},now()+interval '30 days') on conflict(stripe_event_id) do nothing`;}}
