...
export async function queueApprovalNotice(id: string) {
  ...
  for (const [decision,label] of [["approved","Approve plan"],["revision_requested","Revise"],["declined","Decline"]] as const) {
    ...
    buttons[0].push({text:approval.payload.executor==="indicator_release_v1" && decision==="approved" ? "Approve indicator" : publication && decision==="approved" ? `Approve & publish on ${network}` : label,callback_data:`os:${token}`});
  }
