...
export async function readiness() {
  const [evidence, store, buffer, whop] = await Promise.all([
    collectEvidence(),
    (async () => {
      try {
    ...
    (async()=>{ try { return await whopStatus(); } catch { return { configured:Boolean(process.env.WHOP_COMPANY_API_KEY?.trim()), connected:false, companyId:null, companyName:null, ownerUserId:null, publishingConfigured:false, publishChannelId:null, error:"WHOP_CHECK_FAILED" }; } })(),
  ]);
  const ai = {
  ...
  return {
    ...
    evidence, store, ai, coordination: await coordinationStatus(), buffer, whop,
    configuration: { databaseUrlPresent: Boolean(process.env.DATABASE_URL), stripeKeyPresent: Boolean(process.env.STRIPE_SECRET_KEY),
      ...
      privatePublicUrlPresent: Boolean(process.env.AI_OS_PUBLIC_URL), bufferApiKeyPresent: Boolean(process.env.BUFFER_API_KEY), whopApiKeyPresent: Boolean(process.env.WHOP_COMPANY_API_KEY), whopCompanyIdPresent: Boolean(process.env.WHOP_COMPANY_ID), autonomyEnabled: process.env.AI_OS_AUTONOMY_ENABLED === "true",
    ...
    blockers: [
      ...
      ...(!whop.connected ? [whop.error || "WHOP_CONNECTION_UNVERIFIED"] : []),
