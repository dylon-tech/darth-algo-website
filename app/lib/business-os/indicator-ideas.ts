...
export async function syncIndicatorIdeas(day=indicatorDay()) {
  ...
  if(!growth){
    await queueJob("growth","[INDICATOR_IDEAS] Review today's Research brief in indicator_idea_handoffs and its observed source evidence. Select up to three original indicator concepts for the Indicator Builder. For each, give a plain trading name, trader problem, source URLs, actual demand evidence versus hypothesis, differentiated feature, testable closed-bar behavior, intended market/timeframe, acceptance tests, and free/paid hypothesis. Include useful strategy concepts as ideas, never profitability claims. Reject unsupported, duplicate or merely renamed competitor products. Explicitly state BUILD_NONE if no idea has enough evidence. Do not claim Instagram/TikTok coverage if unavailable. Return no tasks or proposals; the server hands your brief to the builder.",`indicator-ideas:${day}:growth`,"schedule");
