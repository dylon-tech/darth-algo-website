...
export const ownerMediaDirection = "The owner handles social video creation. Focus agent creative work on original promotional photos, static graphics and carousels. Do not generate, commission, queue or publish agent-made TikToks, Reels or Shorts, including the rejected 18-second tools promo, unless the owner explicitly changes this direction. Existing website animations are outside this restriction. Competitor video research may continue, but translate supported findings into original static-image concepts and share them with Content and Growth. Standing owner-approved photo standard: premium black/graphite backgrounds with vivid red accents, bold short product-specific hooks, the actual full-color Darth Algo logo on every slide, prominent readable owned indicator captures, explicit TradingView context, useful feature demonstrations and one clear CTA to /links or the relevant product. Use dimensional device/product framing where useful. Reject the retired navy/gold educational layout, tiny chart screenshots, excessive empty space, generic teaching headlines and understated or missing branding. A saved campaign must carry the current creative version before any new publication; template deployment alone is not a cache migration. Never fabricate chart results, winning trades or testimonials, or copy a competitor's finished artwork.";
...
export function creativeDirection(department: Department) {
  const roles:Partial<Record<Department,string>>={
    content:"Use owner_creative_references to write original, product-specific content. Before a new media concept, specify its hook, real asset, short scene copy and CTA. Adapt only to the current executor: xDraft is text-only; do not claim a video/image was generated. Keep a single idea and a short opening. If media is unavailable, create a useful text post rather than repeating an upload request. Use /links for product and company discovery.",
