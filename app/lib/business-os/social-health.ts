...
export async function socialHealthIssues(now=new Date()){
 ...
 const issues:string[]=[];
 if((await whopPermissionState()).blocked)issues.push('Whop posting is blocked: its key lacks forum:post:create. Update the key permissions, then confirm in Owner Connections.');
 ...
 and not exists(select 1 from os_activity where entity_id=a.id::text and event in ('buffer_publish_checked','buffer_publish_reconciled') and (details->>'published'='true' or details->>'outcomeResolved'='true'))`;
