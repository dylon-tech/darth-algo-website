...
export async function liveOverview(): Promise<DeskSnapshot> {
  ...
  const agents: DeskAgent[] = registry.map(role => ({id:role.id,name:names[role.id] || role.id,mandate:role.mandate,configured,latest:latest.find(r=>r.department===role.id)||null,completed:done.find(r=>r.department===role.id)||null,current:requests.find(j=>j.department===role.id&&j.status==='running')||null,next:requests.find(j=>j.department===role.id&&j.status==='queued')||null,waiting:Number(queueCounts.find(r=>r.department===role.id)?.n||0),task:tasks.find(t=>t.department===role.id) as DeskAgent['task'] || null}));
  return {checkedAt:new Date().toISOString(),paused:control[0]?.paused??true,autonomy:process.env.AI_OS_AUTONOMY_ENABLED==='true',scheduler:coordination.heartbeat,budget:coordination.budget,counts:{working:Number(totals[0]?.working||0),queued:Number(totals[0]?.queued||0),completedToday:Number(totals[0]?.completedToday||0),needsOwner:Number(totals[0]?.needsOwner||0)},agents,services:telemetry.services,telemetryAvailable:telemetry.available,decisions:decisions as unknown as DeskSnapshot['decisions'],activity:activity as unknown as DeskSnapshot['activity'],receipts:receipts as unknown as DeskSnapshot['receipts']};
}
