...
export async function readContentQueue(now=new Date()):Promise<ContentQueue>{
 ...
 const items:QueueItem[]=await Promise.all(campaigns.map(async c=>{
  ...
  const state=submitted?'Already submitted — delivery checks apply':canApprove?'Replacement ready for review':decision?.action==='remake'?(revision?.status==='succeeded'?'Brief ready · new artwork still needed':revision?.status==='running'?'Remake in progress':revision?.status==='failed'||revision?.status==='unknown'?'Remake needs attention':'Remake queued'):contentHeld(decision)?'Disapproved · posting stopped':!reviewed?'Creative review required':decision?.action==='approve_replacement'&&decision.reviewHash!==c.review.sha256?'Changed version · posting held':'Scheduled';
