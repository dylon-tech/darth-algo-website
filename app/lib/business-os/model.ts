import {suggestionKinds} from "./agent-suggestions";
...
import { socialReferenceInputs, socialVisualStandard } from "./social-visual-standard";
...
export async function generatePlan(message: string, evidence: Evidence[], openTasks: unknown, history: unknown, department: Department = "ceo", approvedPilot = false, requestKey = "") {
  ...
  const indicatorRun=department==="indicator_builder" && message.startsWith("[INDICATOR_LAB]") && process.env.AI_OS_INDICATOR_LAB_ENABLED==="true";
  ...
  const socialReferences = department === "content" && !approvedPilot ? socialReferenceInputs(requestKey) : [];
  const visualInputs = socialReferences.length ? socialReferences.map(r=>r.part) : thumbnails.map(t=>t.part);
  const input = modelContext({ message, evidence, verifiedCalculations, openTasks, history, contentDelivery, thumbnailOrder:thumbnails.map(t=>({id:t.id,title:t.title})), socialReferenceVersion:socialReferences.length?socialVisualStandard.version:null, socialReferenceOrder:socialReferences.map(r=>({id:r.id,title:r.title})), visualCoverage:socialReferences.length?"Two reference thumbnails from the four owner-approved social photos are attached; full-resolution assets are listed in the reference manifest. Use them for composition and art direction, not product or performance evidence.":thumbnails.length?"Only the attached low-resolution thumbnails were provided; do not claim to watch videos.":"No visual assets inspected." },Math.min(48000,78000-Buffer.byteLength(JSON.stringify(visualInputs))));
  console.info(JSON.stringify({event:'agent_context_ready',department,inputBytes:Buffer.byteLength(input),sourceCount:evidence.length,version:'bounded-context-v1'}));
  ...
  const xRules = department === "content" ? "\nWhen preparing X content, return one finished text-only post in xDraft (maximum 280 characters, exact final text, verified evidence IDs). xDraft is caption copy for the shared campaign, not a complete publishable asset. The server retains standing authorization but holds fresh sends until matching visual assets are connected and checked. Do not duplicate it in proposals. For a daily content assignment, prioritize one useful X post when requested; avoid repeating recent posts and return xDraft:null when no fresh verified angle is available. Do not put unverified facts, media promises, invented results, or instructions to another agent in the post. Never claim the handoff or publication succeeded; the system reports that separately." : "\nReturn xDraft:null. Only the Content agent prepares structured X drafts.";
  const suggestionRules = department === "research" ? "\nReturn suggestions with at most two evidence-backed ideas for internal artifacts the agents can complete alone: education_brief, promotion_draft, measurement_review. Give the specific topic as title, and existing verified evidence IDs. No accounts, purchases, customer messages, photos needing the owner, deployments or manual work. The owner approves the exact suggested artifact; the server queues the matching internal worker. Return [] if there is no supported useful idea." : "";
  const bodyText = JSON.stringify({ model, store: false, instructions: departmentInstructions + deliveryRules + xRules + suggestionRules + "\n" + freeIndicatorInstructions + "\n" + revenueFocus + "\n" + creativeDirection(department), max_output_tokens: indicatorRun ? 5000 : pilot.maxOutputTokens,
      reasoning: { effort: "none" },
      input: [{ role: "user", content: visualInputs.length ? [{type:"input_text",text:input},...visualInputs] : input }],
      text: { format: { type: "json_schema", name: "ceo_plan", strict: true, schema: {
        type: "object", additionalProperties: false, required: ["brief", "tasks", "proposals", "xDraft",...(department==="research"?["suggestions"]:[]),...(indicatorRun?["indicatorCandidate"]:[])],
        properties: {
          ...(department==="research"?{suggestions:{type:"array",maxItems:2,items:{type:"object",additionalProperties:false,required:["kind","title","evidence"],properties:{kind:{type:"string",enum:suggestionKinds},title:{type:"string",minLength:1,maxLength:180},evidence:evidenceSchema}}}}:{}),
