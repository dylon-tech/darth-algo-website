...
export async function publishCommunityPreview(now=new Date()){
 ...
 let posts=rows.filter(r=>isDailySocialPayload(r.payload)&&campaignMatchesReview(r.payload.campaign)&&socialPostUrl(r.details.externalLink,r.payload.network));
 ...
 const attempts=await sql`select entity_id from os_activity where event in ('community_social_started','community_social_sent','community_social_unknown') and entity_id like ${day+'%'}`;
 const attempted=new Set(attempts.map(r=>r.entity_id));
 posts=posts.filter(r=>!attempted.has(campaignKey(r.payload.campaign)));
 if(!posts.length)return {posted:false,reason:'already_attempted'};
 posts.sort((a,b)=>String(a.payload.campaign.slot).localeCompare(String(b.payload.campaign.slot)));
 const key=campaignKey(posts[0].payload.campaign);
 posts=posts.filter(r=>campaignKey(r.payload.campaign)===key);
 posts.sort((a,b)=>['instagram','x','threads'].indexOf(a.payload.network)-['instagram','x','threads'].indexOf(b.payload.network));
