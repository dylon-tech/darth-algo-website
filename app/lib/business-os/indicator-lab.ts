...
export async function indicatorDecisionMessage(id:string,decision:string){
  ...
  return decision==="approved"?"Release package approved: indicator, instruction image and educational post. Saved in the publication queue. TradingView publishing worker is not connected yet; nothing has been published.":decision==="revision_requested"?"Revision saved. The Lab will use your notes in the next available daily slot; revised code needs a new approval.":"Indicator declined. It will not be released.";
}
export async function recordIndicatorRelease(id:string,hash:string,url:string,checks:{compiled:boolean;replay:boolean;notes:string;screenshotUrl:string;educationUrl:string;packageHash:string}){
  ...
  await db().begin(async tx=>{
    ...
    if(!p || p.hash!==checks.packageHash || c.payload.packageHash!==p.hash || fingerprint({sourceHash:p.sourceHash,preview:p.preview,education:p.education})!==p.hash)throw Error("APPROVED_PACKAGE_REQUIRED");
