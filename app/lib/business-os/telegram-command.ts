...
async function handleUpdate(update: OwnerUpdate) {
  ...
  if(departments.includes(selected as Department)){
    ...
    if(menu?.assignmentId){
      ...
      if(department==="content" && menu.assignmentId==="daily-content-engine")message="Prepare one fresh, useful X post in xDraft with a verified Darth Algo links-page or community CTA. It goes to the routine automatic publishing queue. In the brief, include the actual post and explain its angle in one sentence. Avoid repeating existing posts. No invented results, prices or claims.";
