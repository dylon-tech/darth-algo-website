...
import {pineHash,pineChecks,validPrivatePreview,indicatorReleasePolicy} from "./indicator-policy";
...
export async function queueApprovedIndicator(tx:TransactionSql,id:string,payload:Record<string,string>) {
  ...
  if(c && (c.candidate.tier!=="free" || c.candidate.monthlyPriceUsd!==0))throw Error("FREE_INDICATOR_REQUIRED");
  if(!c || c.approval_id!==id || !p || p.hash!==payload.packageHash || fingerprint({sourceHash:p.sourceHash,preview:p.preview,education:p.education,publication:p.publication})!==p.hash || fingerprint(p.publication)!==fingerprint(indicatorReleasePolicy) || pineHash(c.candidate.pine)!==payload.sourceHash || !validPrivatePreview(c.private_preview,payload.sourceHash) || fingerprint(c.private_preview)!==fingerprint(p.preview))throw Error("COMPLETE_INDICATOR_PACKAGE_REQUIRED");
...
export async function prepareIndicatorPackage(id:string,sourceHash:string,input:unknown) {
  ...
  const approvalId=await db().begin(async tx=>{
    ...
    const releasePackage={sourceHash,preview:c.private_preview,education,publication:indicatorReleasePolicy};
