...
import {capabilityInventory,ownerActions,instagramProviderObservation} from './inventory';
import {message,platforms,validateMessage,type Platform} from './policy';
import {productionAdapters,runWelcomeQueue} from './engine';
// A one-time, dated UI observation. Never treated as a live provider heartbeat.
export async function recordInstagramDraft(){
 await ensureWelcomeSchema();const sql=db();const observation=instagramProviderObservation;
 await sql.begin(async tx=>{
  const inserted=await tx`insert into os_welcome_checks(id,details,checked_at) values(${observation.id},${sql.json(observation)},${observation.checkedAt}) on conflict(id) do nothing returning id`;
  if(!inserted.length)return;
  // Preserve a subsequently configured sender. Workspace ID is NOT a Meta account ID.
  await tx`update os_welcome_platforms set sending_owner='provider',provider='Manychat',connected=true,paused=true,eligible=false,mode='keyword_draft',evidence=${sql.json(observation)},updated_at=now() where platform='instagram' and sending_owner='none'`;
  await tx`update os_tasks set result='Instagram connected in Manychat; Follow-to-DM unavailable. Native exact WELCOME draft saved but inactive. Owner handles promo; controlled messaging test and final offer validation pending.' where dedupe_key='DA-WELCOME-20260923-v1' and status='blocked'`;
 });
}
export async function verifyWelcomeDiscount(){
 ...
 const details={ready:false,code:'WELCOME',active:p?.active===true,livemode:p?.livemode===true,percentOff:c?.percent_off??null,duration:c?.duration??null,firstTimeOnly:p?.restrictions.first_time_transaction??null,maxRedemptions:c?.max_redemptions??null,redeemed:c?.times_redeemed??null,expiresAt:p?.expires_at??null,productRestrictions:c?.applies_to?.products??null,reason:!p||!c?'WELCOME_NOT_VERIFIED':c.percent_off!==25?'DISCOUNT_PERCENT_MISMATCH':c.duration!=='once'?'DISCOUNT_DURATION_MUST_BE_ONCE':p.restrictions.first_time_transaction!==true?'FIRST_TIME_RESTRICTION_REQUIRED':c.max_redemptions!==null||p.max_redemptions!==null?'GLOBAL_REDEMPTION_CAP_PRESENT':p.expires_at!==null||c.redeem_by!==null?'EXPIRATION_CONFLICT':c.applies_to?.products?.length?'PRODUCT_RESTRICTION_CONFLICT':'CHECKOUT_AND_TRIAL_TEST_REQUIRED',checkedAt:new Date().toISOString()};
...
export async function welcomeSnapshot(){
 await recordInstagramDraft();const sql=db();
...
export async function welcomeTick(){
 ...
 return {status:'blocked',reason:'NATIVE_DRAFT_TEST_AND_OFFER_VALIDATION_PENDING',sendingAdapters:productionAdapters.length,providerObservation:instagramProviderObservation.id};
}
export async function welcomeBrief(){try{const s=await welcomeSnapshot();const alerts=s.outcomes.filter(o=>['failed','unknown'].includes(o.status)).reduce((n,o)=>n+o.n,0);return `Welcome Agent · ${s.platforms.map(p=>p.platform+': '+p.status).join(' · ')}\nOffers accepted: unavailable · paid customers: unavailable · attributed revenue: unavailable\n${alerts?alerts+' failed/unknown sends need review. ':''}Instagram: native WELCOME draft; follower trigger unavailable. Needs you: owner-controlled test account and final offer terms — /owner/welcome.`;}catch{return 'Welcome Agent · status unavailable; no sending is assumed.';}}
