...
const schema=(await readFile('app/lib/business-os/schema.ts','utf8')).match(/export const schema = `([\s\S]*?)`;/)[1];await sql.unsafe(schema);await sql`update os_control set paused=false`;
...
try{
 ...
 for(const engine of ['chromium','webkit']){
  ...
  await page.getByRole('button',{name:'Give work',exact:true}).click();await page.getByRole('status').filter({hasText:/Request .* saved/}).waitFor();
