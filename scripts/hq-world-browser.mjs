...
try{
 ...
 for(const engine of ['chromium','webkit']){
  ...
  failFeed=true;await page.getByRole('button',{name:'Refresh headquarters'}).click();await page.getByRole('alert').filter({hasText:'Business records could not be refreshed'}).waitFor();assert.ok(await page.getByRole('button',{name:/System health/}).getByText('Unknown',{exact:true}).isVisible());failFeed=false;await page.getByRole('button',{name:'Refresh headquarters'}).click();await page.getByRole('alert').filter({hasText:'Business records could not be refreshed'}).waitFor({state:'hidden'});
